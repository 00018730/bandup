...
// Cell is either plain text or a blank (with a question number)
...
interface Question {
  ...
  correct_answer: string; answer_location: string; sort_order: number;
}
interface Test {
  id: number; title: string; passage: string;
  extra_data: { table: TableData };
  questions: Question[];
}
...
export default function TableCompletionScreen({ route, navigation }:any) {
...
const [completedCount, setCompletedCount] = useState(0);
  ...
  const fetchTest = async () => {
    ...
    const { data, error } = await supabase
      .from('tests').select(`id, title, passage, extra_data, questions(id, body, prefix, suffix, correct_answer, answer_location, sort_order)`)
      .eq('id', testId).single();
  ...
  const renderTable = (reviewMode: boolean) => {
    ...
    return (
      <View style={[styles.tableWrap,{borderColor:colors.border}]}>
        {/* Header row */}
        <View style={[styles.tableRow,styles.tableHeaderRow,{backgroundColor:isDark?'#2a1a12':'#fff0eb'}]}>
          {headers.map((h,i)=>(
        ...
        {rows.map((row,ri)=>(
          <View key={ri} style={[styles.tableRow,{borderTopColor:colors.border,borderTopWidth:1}]}>
            {row.map((cell,ci)=>{
              ...
              // Blank cell
              const q = qMap[cell.blank];
              if (!q) return <View key={ci} style={styles.tableCell}/>;
                ...
                <View key={ci} style={[styles.tableCell,!isLast&&{borderRightColor:colors.border,borderRightWidth:1},styles.blankCell]}>
                  ...
                  {reviewMode ? (
                    <View style={[styles.reviewBubble,{
                      ...
                      <Text style={{fontSize:12,fontWeight:'800' as const,color:isCorrect?GREEN:ORANGE}}>{userAns||'—'}</Text>
                      {!isCorrect&&<Text style={{fontSize:11,fontWeight:'700' as const,color:GREEN}}>{' '}({q.correct_answer})</Text>}
                    </View>
                  ) : (
                    <TextInput
                      style={[styles.tableInput,{backgroundColor:colors.input,borderColor:answers[q.id]?ORANGE:colors.border,color:colors.text}]}
                      placeholder="___" placeholderTextColor={colors.muted}
    ...
    <SafeAreaView style={[styles.safe,{backgroundColor:colors.bg}]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" automaticallyAdjustKeyboardInsets={true}>
        ...
        <View style={[styles.instructBox,{backgroundColor:isDark?'#2a1a12':'#fff8f6',borderColor:ORANGE}]}>
          <Text style={[styles.instructText,{color:colors.text}]}>Complete the table. Choose <Text style={{color:ORANGE,fontWeight:'800'}}>ONE WORD ONLY</Text> from the passage for each answer.</Text>
        </View>
        {renderTable(false)}
